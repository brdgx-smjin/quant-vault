#!/usr/bin/env python3
"""Walk-Forward grid test for 30m BTC/USDT:USDT — ATR SL x MTF filter.

Tests 6 combinations:
- ATR SL multiplier: [1.5, 2.0, 2.5]
- Multi-TF filter: [off, on]
Each combination runs WF with 3 and 5 windows.

30m-adjusted parameters:
- max_hold_bars=96 (48 hours, vs 48 bars=8 days on 4h)
- warmup_bars=200 (200 x 30m = ~4 days, sufficient for EMA_200)
"""
...
file_h = logging.FileHandler("logs/wf_30m.log")
...
def main() -> None:
    """Run Walk-Forward grid on 30m data."""
    # Load 30m data
    parquet_path = f"{DATA_DIR}/processed/{SYMBOL.replace('/', '_').replace(':', '_')}_30m.parquet"
    if not Path(parquet_path).exists():
        logger.error("30m data not found at %s", parquet_path)
        logger.error("Run scripts/fetch_30m_data.py first.")
        sys.exit(1)
    ...
    # Resample 30m -> 4h for MTF filter
    df_4h = DataPreprocessor.resample(df, "4h")
    df_4h = BasicIndicators.add_all(df_4h)
    df_4h = df_4h.dropna()
    logger.info("4h resampled: %d bars (%s ~ %s)",
                len(df_4h), df_4h.index[0], df_4h.index[-1])

    engine = BacktestEngine(initial_capital=10_000, max_hold_bars=96)
    ...
    for atr_mult in ATR_SL_MULTS:
        for use_mtf in MTF_OPTIONS:
            ...
            def make_factory(mult: float, mtf: bool):
                """Create a factory closure for the current grid params."""
            ...
            htf_arg = df_4h if use_mtf else None
    ...
    logger.info("")
    logger.info("=" * 80)
    logger.info("  GRID RESULTS SUMMARY")
    logger.info("=" * 80)
    logger.info(
    ...
    if best_result:
        ...
        logger.info("")
        logger.info("  Recommended live_trading.py settings:")
